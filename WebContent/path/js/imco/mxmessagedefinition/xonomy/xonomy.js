	...
	if(selectedRowId != null && selectedRowId != undefined){
		...
		if(selectedRowObject != ""){
			...
			for(var i=0; i<tagRows.length-1; i++){
				...
				$("#xmlDialogEditor_"+_pageRef).find(eval(parentList)).find('.opening:first').attr("style","background:"+cols[3]+"");
			}//end loop
		}//end if
	}//end if
	
	//Color in Dialog end
	//Start adding message identifier
	var msgId = $("#xmlDialogEditor_"+_pageRef).children('div').attr('data-name');
	if(msgId != null && msgId != '' && msgId != undefined)
	{
		var msgData = {};
		var tagId = $("#xmlDialogEditor_"+_pageRef).children('div').attr('id');
		msgData['TAG_NAME'] = msgId;
		msgData['SELECTED_XML'] = $("#selectedMessage_"+_pageRef).val();
		msgData['HTML_ID'] = tagId;
		msgData['TAG_COLOR'] = '#d0e5f5';
		
		var rowId = "new_"+(new Date()).getTime();
		jQuery("#xmlDialogGrid_Id_"+_pageRef).jqGrid('addRowData', rowId, msgData, 'last');		
		//$("#"+tagId).find('.opening:first').attr("style","background:#d0e5f5");
		$("#"+rowId).hide();
	}
	//End adding message identifier
...
Xonomy.renderElement=function(element) {
	...
	html+='<div data-name="'+element.name+'" id="'+htmlID+'" class="'+classNames+'">';
		...
		html+='<span class="tag opening" style="background-color: '+spec.backgroundColour+';">';
			...
			html+='</span>';
			//html+='<span class="rollouter" onclick="Xonomy.click(\''+htmlID+'\', \'rollouter\')"></span>';shah
			html+='<span class="punc slash">/</span>';
			html+='<span class="punc">&gt;</span><span style="float: right;" class="name">';
				//+'<img src="/imal_consolidation_portal/xonomy/menu.png" style="margin-top: -4px;" />';
		html+='</span></span>';
		...
		html+='</div>';
		//html+='<br /><a onclick="Xonomy.newElementChild(\''+htmlID+'\', \'\')" style="width: 25px; margin-top: -17px;" type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">+</a>';
		//html+='<br /><a onclick="Xonomy.newElementChild(\''+htmlID+'\', \'\')" href="#"><img style="margin-top: -17px;" src="/imal_consolidation_portal/xonomy/add_btn.png" /></a>';
...
Xonomy.newAttribute=function(htmlID, parameter) {
	...
	document.body.appendChild(Xonomy.makeBubble(Xonomy.askString(''))); //create bubble
	Xonomy.showBubble($(div)); //anchor bubble to element
	Xonomy.answer=function(val) {
		if(val != null && val != "" && val !=" "){
			var $element=$("#"+htmlID);
			var html=Xonomy.renderAttribute({type: "attribute", name: val, value: parameter.value}, $element.data("name"));
...
Xonomy.newElementChild=function(htmlID, parameter) {
	...
	Xonomy.answer=function(val) {
		if(val != null && val != "" && val !=" "){
			var tag = "<"+val+"></"+val+">";
			var jsElement=Xonomy.harvestElement(document.getElementById(htmlID));
			var html=Xonomy.renderElement(Xonomy.xml2js(tag, jsElement));
			var $html=$(html).hide();
